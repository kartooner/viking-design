# Viking design framework 🛡️

The **Viking design framework** is an AI-intent-first UX product design methodology. It combines the aesthetic rigor of the Impeccable framework with strategic "Design with Intent" principles, powered by a sequential chain of specialized agents.

## 📖 Methodology

Viking follows a disciplined **Systems First, Surfaces Second** philosophy:

1.  **The 5 Gates:** Determine if AI is the right tool before building (Value, Precision, Logic, Risk, Context).
2.  **Things, States, Rules:** Map the logical object model before touching the canvas.
3.  **The High Council:** A sequential 1-7 skill chain that guides you from strategy to handoff.
4.  **Generative UI Recipes:** Contextual surfaces designed as recipes rather than fixed screens.
5.  **The AI Slop Test:** A quality audit to reject generic, AI-generated aesthetics in favor of clean, typographically beautiful design.

## 🏛️ The High Council (agents)

1.  **Odin (Creative Director):** Strategic lead and aesthetic guardian.
2.  **Mimir (UX Researcher):** The wise one who runs the 5 Gates fit assessment.
3.  **Tyr (Creative Strategist):** Framer of problems and opportunity sizer.
4.  **Heimdall (Systems Architect):** Watcher who maps Things, States, and Rules.
5.  **Loki (Flow Designer):** Designer of contextual interaction recipes.
6.  **Bragi (Content Designer):** Poet who crafts the Sandwich Pattern copy.
7.  **Thor (Handoff Specialist):** Deliverer of component contracts to Engineering.

## 🚀 Installation

Add the Viking design framework to your Claude Code or Gemini CLI environment:

```bash
npx skills add kartooner/viking-design
```

## 🛠️ Usage

Type these as slash commands at the AI prompt:

- `/viking-design` — Open the Discovery Map.
- `/viking-design gate` — Run the 5 Gates fit assessment.
- `/viking-design frame` — Problem framing: design brief, user hats, opportunity sizing.
- `/viking-design map` — Logic mapping: Things, States, and Rules.
- `/viking-design chain` — Activate the full Council chain from strategy to handoff.
- `/viking-design content` — UX copy using the Sandwich Pattern.
- `/viking-design audit` — Run the AI Slop Test quality audit.

---

### 📜 Attribution & intellectual property

The **Viking design framework** is a unique synthesis and agentic implementation created by **Erik Sagen**. It is built upon the foundational concepts of the following open-source projects and methodologies:

*   **[Impeccable Design Framework](https://github.com/impeccable-design):** Source of the rigorous visual laws, token-normative standards, and "AI Slop Test" methodology.
*   **[Design with Intent](https://designwithintent.ai/):** Source of the "Intent-First" strategic philosophy and behavioral mapping principles.

While Viking leverages these established standards, the **5 Gates**, the **Things, States, Rules** orchestration logic, and the specific **Nordic High Council** agentic chain are original contributions by Erik Sagen.

### ⚖️ License

Distributed under the **MIT License**. See `LICENSE` for more information.
